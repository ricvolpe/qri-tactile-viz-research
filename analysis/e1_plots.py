...
items = pd.read_csv("data/e1_sine_waves_items.csv")
questions = pd.read_csv("data/e1_sine_waves_questions.csv")
data = items.join(questions, on="question_id", rsuffix="_Q")

metrics = ["pleasant", "stimulating", "complex"]
colors = ["y", "r", "b"]
key = "user_id"
dimensions = ["spatial_waveform", "temporal_waveform"]
...
for d in dimensions:
    criteria = (
        (stats["spatial_waveform"] == 35)
        if d == "temporal_waveform"
        else (stats["temporal_waveform"] == 4)
    )
    ...
    ax.set_xlabel(d.replace("_", " ").capitalize())
    fig.savefig(f"analysis/figures/e1_{d}.png")
